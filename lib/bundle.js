...
function removeSoldier(move, gameProperties) {
    ...
    var removingPiece = move.TURN === common.PURPLE_TURN ? gameProperties.PURPLE_PLAYER : gameProperties.YELLOW_PLAYER;
...
function scoreBoard(turn, board, gameProperties) {
    return 10 * countN(true, 2, turn, board) + 10 * countN(false, 2, turn, board) + 100 * countN(true, 3, turn, board) + 100 * countN(false, 3, turn, board) + (turn === common.YELLOW_TURN ? gameProperties.YELLOW_PLAYER.PLACED - gameProperties.PURPLE_PLAYER.PLACED : gameProperties.PURPLE_PLAYER.PLACED - gameProperties.YELLOW_PLAYER.PLACED);
}
...
function minimax(board, depth, maxPlayer, turn, phase1, gameProperties) {
    ...
    if (maxPlayer) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            var value = minimax(child, depth - 1, false, (turn + 1) % 2, phase1, gameProperties);
            if (value > bestValue) {
                bestValue = value;
            }
        ...
        for (var _i = 0; _i < _children.length; _i++) {
            ...
            var _value = minimax(_child, depth - 1, true, (turn + 1) % 2, phase1, gameProperties);
            if (_value < _bestValue) {
                _bestValue = _value;
                bestBoards = [];
            }
            bestBoards.push(_child);
        }
        return _bestValue;
    }
}
    ...
    for (var row = 0; row < MATRIX_SIZE; row++) {
        for (var col = 0; col < MATRIX_SIZE; col++) {
            if (phase1) {
                ...
                if (board[row][col].ISAVAILABLE && board[row][col].TURN === turn) {
                    var shiftChildren = getShiftChildren(board, turn, row, col);
