...
function scoreBoard(turn, board, gameProperties) {
    ...
    return countN(true, 2, turn, board) + countN(false, 2, turn, board) + 10 * countN(true, 3, turn, board) + 10 * countN(false, 3, turn, board) + (turn === common.YELLOW_TURN ? gameProperties.YELLOW_PLAYER.PLACED - gameProperties.PURPLE_PLAYER.PLACED : gameProperties.PURPLE_PLAYER.PLACED - gameProperties.YELLOW_PLAYER.PLACED);
}
...
function minimax(board, depth, maxPlayer, turn, phase1, gameProperties) {
    ...
    if (maxPlayer) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            var m = minimax(child.BOARD, depth - 1, false, (turn + 1) % 2, phase1, child.PROPERTIES);
        ...
        for (var _i = 0; _i < _children.length; _i++) {
            ...
            var _m = minimax(_child.BOARD, depth - 1, true, (turn + 1) % 2, phase1, _child.PROPERTIES);
...
function phase1WithComputer() {
    while (GAME_PROPERTIES.PURPLE_PLAYER.AVAILABLE > 0 || GAME_PROPERTIES.YELLOW_PLAYER.AVAILABLE > 0) {
        if (computerTurn) {
            var bestM = minimax(board, 3, true, GAME_PROPERTIES.TURN, true, GAME_PROPERTIES);
            board = bestM.BOARD;
...
function phase2WithComputer() {
    while (GAME_PROPERTIES.PURPLE_PLAYER.PLACED > 2 && GAME_PROPERTIES.YELLOW_PLAYER.PLACED > 2) {
        if (computerTurn) {
            var bestM = minimax(board, 3, true, GAME_PROPERTIES.TURN, false, GAME_PROPERTIES);
            board = bestM.BOARD;
            GAME_PROPERTIES = bestM.PROPERTIES;
            GAME_PROPERTIES.TURN = (GAME_PROPERTIES.TURN + 1) % 2;
            printBoard();
