...
function phase3(positions) {
    ...
    var endPositions = GAME_PROPERTIES.TURN === common.YELLOW_TURN ? prompt("Yellow: Enter position to fly to in the form of row,col") : prompt("Purple: Enter position to fly to in the form of row,col");

    endPositions = endPositions.split(",");

    var startRow = parseInt(positions[0], 10);
    var startCol = parseInt(positions[1], 10);
    ...
    var endRow = parseInt(endPositions[0], 10);
    var endCol = parseInt(endPositions[1], 10);
    var move = {
        ROW: endRow,
        COL: endCol,
        TURN: GAME_PROPERTIES.TURN,
                ...
                if (board[row][col].ISAVAILABLE && board[row][col].TURN === turn) {
                    if (turn === common.YELLOW_TURN && gameProperties.YELLOW_PLAYER.PLACED === 3 || turn === common.PURPLE_TURN && gameProperties.PURPLE_PLAYER.PLACED === 3) {
                        ...
                        for (var _i2 = 0; _i2 < 4; _i2++) {
                            ...
                            var _move2 = { ROW: row, COL: col, BOARD: _copyBoard2, TURN: turn };

                            _move2.SHIFT = _i2;
                            if (shiftSoldier(_move2, _copyGameProperties2)) {
                                // Update row and col for handleNewMills
                                _move2.ROW = _move2.SHIFTROW;
                                _move2.COL = _move2.SHIFTCOL;
                                handleNewMillsComputer(_move2, _copyGameProperties2);
