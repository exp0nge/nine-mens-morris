...
function isValidShift(move) {
    ...
    for (var i = 0; i < VALID_SHIFTS[row][move.COL].length; i++) {
        var element = VALID_SHIFTS[row][move.COL][i];
...
function alertIfWinner() {
    if (_common.PURPLE_PLAYER.PLACED <= 2 && _common.PURPLE_PLAYER.AVAILABLE <= 0 || _common.YELLOW_PLAYER.PLACED <= 2 && _common.YELLOW_PLAYER.AVAILABLE <= 0) {
        clearElement(turnPromptText);
        clearElement(turnText);
        document.getElementById("phaseText").innerHTML = "WINNER " + (_common.PURPLE_PLAYER.AVAILABLE <= 2 ? "YELLOW" : "PURPLE");
...
function phaseTwoHandler(e) {
    ...
    if (GAME_PROPERTIES.SOURCE === null) {
        ...
        var move = (0, _common.makeMoveProp)(x_original, y_original, GAME_PROPERTIES.TURN, true, x, y, board);
        if (shiftSoldier(move)) {
